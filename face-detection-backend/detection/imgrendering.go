...
var (
	logos     []image.Image
	logosPath = []string{"hat.png", "hat.png"}
	datadir string
...
func (r *RenderedImage) drawFunFace(face *opencv.Rect, num int, cvimage *opencv.IplImage) {
  ...
  logo := resize.Resize(0, uint(face.Height()-face.Height()/10), logos[num], resize.NearestNeighbor)
	logorect := image.Rect(face.X()+face.Width()/2-logo.Bounds().Dx()/2-logo.Bounds().Dx()/8,
		face.Y()-logo.Bounds().Dy()/2,
